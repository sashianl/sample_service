...
Note that usage of the administration flags will be logged by the service.
    ...
    def create_samples(self, ctx, params):
        """
        Create a new sample or a sample version.
        :param params: instance of type "CreateSamplesParams" -> structure:
           parameter "samples" of list of type "SampleSave" -> structure:
           ...
           boolean value, 0 for false, 1 for true.), parameter "as_user" of
           type "user" (A user's username.)
        :returns: instance of list of type "SampleAddress" (A Sample ID and
           version. id - the ID of the sample. version - the version of the
        ...
        for samp in params['samples']:
          ...
          ret = self._samples.save_sample(
              sample, user if user else _UserID(ctx[_CTX_USER]), id_, prev_ver, as_admin=as_admin)
          addresses.append({'id': str(ret[0]), 'version': ret[1]})
        #END create_samples

        # At some point might do deeper type checking...
        if not isinstance(addresses, list):
            raise ValueError('Method create_samples return value ' +
                             'addresses is not type list as required.')
        # return the results
        return [addresses]
